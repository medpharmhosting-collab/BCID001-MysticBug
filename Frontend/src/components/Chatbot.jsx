...
const ChatBot = ({ bottom = '1.5rem', open = false, initialOpen = true, showButton = true }) => {
    ...
    <>
      {!botPopup && showButton && (
        <button
          ...
          style={{ bottom: bottom }}
          className="fixed top-[520px] right-4 sm:right-6 w-20 h-20 sm:w-28 sm:h-28 rounded-full hover:scale-110 transition-all duration-300 flex items-center justify-center z-50 bg-transparent"
        >
          <img src={default_page_images.bot} alt="" />
        </button>
      )}

      <div className={`fixed inset-x-0 bottom-0 sm:inset-auto sm:bottom-6 sm:right-6 w-full sm:w-96 h-[100dvh] sm:h-[600px] bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col z-[150] border border-gray-200 transition-all duration-500 ease-out ${botPopup ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full'}`}>
